...
import { deriveLayer } from '../model/derive.ts';
import { layoutGraph } from './index.ts';
import type { Model, LayerId } from '../model/types.ts';
...
export async function freezeLayout(
  ...
  const layout: Record<string, { nodes: Record<string, { x: number; y: number }> }> = { ...(model.layout ?? {}) };
  let count = 0;
...
function spliceLayout(source: string, layout: Record<string, unknown>): string {
...
function renderLayoutBlock(layout: Record<string, unknown>): string {
