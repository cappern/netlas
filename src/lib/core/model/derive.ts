...
import { ifaceVlans, networkOf } from './validate.ts';
import type { Model, Device, External, LayerId } from './types.ts';

/**
 * The neutral graph shape every layer compiles to. Kept deliberately loose:
 * the derivation stuffs layer-specific facts into `detail`, and layout and the
 * renderers only rely on the common fields declared here.
 */
 ...
 */
export const LAYERS: LayerId[] = ['l1', 'l2', 'l3'];

/** ...plus the layers that exist only when the model carries the facts. */
export const ALL_LAYERS: LayerId[] = [...LAYERS, 'dep'];
 ...
 * depends on anything — and the same argument applies to L2 and L3. A pure
 * cabling record has no routing; emitting the tab anyway would assert an
 * absence the model never stated.
 *
 * L1 is the exception: a device is itself a physical fact, so any device makes
 * L1 drawable even before a single cable is drawn. This keeps a design from
 * going blank the moment its last link is removed — the boxes are still real,
 * they just have nothing plugged in yet.
 */
export function layersFor(model: Model): LayerId[] {
  const has: Record<LayerId, boolean> = {
    l1: (model.devices ?? []).length > 0,
...
export function deriveLayer(model: Model, layer: LayerId): Graph {
  switch (layer) {
...
function tierOf(device: Device & { tier?: number }): number {
  return device.tier ?? ROLE_TIER[device.role] ?? 6;
}

function zoneGroups(model: Model, memberIds: string[]): GraphGroup[] {
...
function deviceNode(d: Device & { stack?: number }): GraphNode {
  const badges: string[] = [];
...
function deriveL1(model: Model): Graph {
  const ix = indexModel(model);
  const nodes = model.devices.map((d) => deviceNode(d));
  const byLag = new Map<string, GraphEdge>();
  ...
  const sharedVlans = (a: { device: string; iface: string | null }, b: { device: string; iface: string | null }): number[] => {
  ...
  model.links.forEach((link, n) => {
    const l = link as typeof link & { lag?: string; label?: string };
    ...
    if (key && byLag.has(key)) {
      ...
      const memberLinks = e.detail.memberLinks as Array<Record<string, unknown>>;
      memberLinks.push({
        aPort: a.iface,
        bPort: b.iface,
        speed: l.speed,
...
interface L2Entry { ports: string[]; tagged: boolean; svi: boolean; }

function deriveL2(model: Model): Graph {
  const ix = indexModel(model);
  const members = new Map<number, Map<string, L2Entry>>(); // vlanId -> deviceId -> entry
  ...
  const nodes = model.devices.filter((d) => usedDevices.has(d.id)).map((d) => deviceNode(d));
  const edges: GraphEdge[] = [];
...
function deriveL3(model: Model): Graph {
  const ix = indexModel(model);

  const networks = new Map<string, GraphNode>(); // cidr -> node
  const addNetwork = (cidr: string, meta: Record<string, unknown>): GraphNode => {
    const net = networkOf(cidr);
    if (!networks.has(net)) {
      networks.set(net, {
        id: `net:${net}`,
        label: net,
        sublabel: (meta?.name as string) ?? '',
        kind: 'network',
        role: 'network',
        vendor: 'generic',
        tier: 4,
        badges: meta?.vrf ? [meta.vrf as string] : [],
  ...
  for (const v of model.vlans) {
    if (v.subnet) addNetwork(v.subnet, { name: v.name, vlan: v.id, gateway: v.gateway, vrf: (v as { vrf?: string }).vrf, zone: v.zone });
  }
  ...
  const edges: GraphEdge[] = [];
  const routedDevices = new Map<string, Device | undefined>();
  ...
  for (const d of model.devices) {
    ...
    for (const i of routed) {
      ...
      const nd = net.detail as { gateway?: string; attached: Array<Record<string, unknown>>; cidr: string; vrf?: string; name?: string };
      const isGateway = nd.gateway && i.ip!.split('/')[0] === nd.gateway;
      nd.attached.push({ device: d.id, iface: i.name, ip: i.ip, gateway: !!isGateway });
      edges.push({
        id: `l3-${d.id}-${i.name}`,
        a: d.id,
        b: net.id,
        ...
        vrf: (i as { vrf?: string }).vrf,
        directed: false,
        kind: 'attachment',
        detail: {
          cidr: nd.cidr,
          gateway: !!isGateway,
          vrf: (i as { vrf?: string }).vrf ?? nd.vrf,
          networkName: nd.name,
        },
      });
    }
  ...
  const nodes: GraphNode[] = [
    ...[...routedDevices.values()].filter((d): d is Device => Boolean(d)).map((d) => deviceNode(d)),
    ...networks.values(),
  ];

  const vrfs = new Map<string, GraphGroup>();
  for (const n of nodes) {
    const vrf = n.detail?.vrf as string | undefined;
    if (!vrf) continue;
...
function externalNode(e: External & { tags?: string[] }, consumes: boolean): GraphNode {
  return {
    id: e.id,
    label: e.label ?? e.id,
...
function deriveDep(model: Model): Graph {
  const ix = indexModel(model);
  ...
  const nodes: GraphNode[] = [
    ...model.devices.filter((d) => holders.has(d.id)).map((d) => deviceNode(d)),
    ...(model.externals ?? [])
  ...
  const labelFor = (services: { name: string }[], kind: string): string => {
  ...
  deps.forEach((dep, n) => {
    ...
    if (merged.has(key)) {
      ...
      const svcList = e.detail.services as { name: string }[];
      const descList = e.detail.descriptions as string[];
      if (service) svcList.push(service);
      if (dep.description) descList.push(dep.description);
      e.label = labelFor(svcList, dep.kind);
      return;
    }
...
function legendFromEdges(edges: GraphEdge[], key: 'media'): { kind: string; label: string }[] {
