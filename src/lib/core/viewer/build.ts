...
import { themeToCss } from '../theme/themes.ts';
import type { Theme } from '../theme/themes.ts';
import { esc } from '../render2d/svg.ts';
import { buildImpactMap } from '../model/impact.ts';
import type { Model, LayerId, Diagnostic } from '../model/types.ts';
import type { Graph } from '../model/derive.ts';
import type { Placed } from '../layout/index.ts';

/** One rendered layer as it enters the viewer builder. */
...
export interface ViewerArgs {
  ...
  enable3d?: boolean;
  warnings?: Diagnostic[];
  /** All themes, so the viewer can offer a theme switcher. */
  themes?: Record<string, Theme>;
  /** Cross-design link hrefs, keyed by the node they leave from (workspace export). */
  links?: { node: string; href: string }[];
}
 ...
 */
export function renderViewer({ model, layers, theme, enable3d = true, warnings = [] }: ViewerArgs): string {
  ...
  const tabs = layers
    .map(({ graph }) => {
      ...
      return (
        ...
        `<strong>${(graph.layer as string) === 'iso' ? 'ISO' : graph.layer.toUpperCase()}</strong>` +
...
<script>window.NETLAS = JSON.parse(document.getElementById('nd-payload').textContent);</script>
<script>${JS}</script>
</body>
</html>`;
}
...
function layerWord(layer: string): string {
  const words: Record<string, string> = { l1: 'Physical', l2: 'VLANs', l3: 'Routing', iso: 'Isometric', dep: 'Dependencies' };
  return words[layer] ?? layer;
}

/** Safe to embed inside <script type="application/json">. */
function jsonForHtml(obj: unknown): string {
