import { icon, VENDOR_MARK } from './icons.ts';
import { dashFor, weightFor, DATA_FONT, detailFlags } from '../theme/themes.ts';
import type { Theme, DetailFlags } from '../theme/themes.ts';
import { groupHulls } from '../layout/index.ts';
import type { Placed, PlacedBox, Point, GroupHull } from '../layout/index.ts';
import type { Graph, GraphNode, GraphEdge } from '../model/derive.ts';

/** A positioned label chip awaiting collision resolution. */
interface Chip {
  text: string;
  cx: number;
  cy: number;
...
/** Options common to the SVG renderer. */
...
export function renderSvg(graph: Graph, placed: Placed, theme: Theme, opts: RenderOpts = {}): string {
  const {
    titleBlock = true,
    legend = true,
    padding = 40,
...
function zoneHull(g: GroupHull, theme: Theme): string {
  ...
  return (
    ...
    `font-size="10.5" font-weight="600" letter-spacing="0.09em" fill="${z.text}">${esc(label.toUpperCase())}</text>` +
    `</g>`
  );
}
...
function edge(e: GraphEdge, pts: Point[], theme: Theme, interactive: boolean, show: DetailFlags = detailFlags('full')): string {
...
function edgeLabels(e: GraphEdge, pts: Point[], theme: Theme): string[] {
  const out: string[] = [];
  const chips: Chip[] = [];
...
function collectChips(out: Chip[], e: GraphEdge, pts: Point[], theme: Theme, show: { chip: boolean } = { chip: true }): void {
 ...
 */
function offsetOffCable(spec: Chip, dx: number, dy: number): void {
 ...
 */
function resolveChips(chips: Chip[], iterations = 28): Chip[] {
...
function portChip(text: string, at: Point, toward: Point, theme: Theme, media: string): Chip {
...
function chipSpec(
  ...
  cy: number,
  theme: Theme,
  { fill, stroke, text: color, size, mono, axis, detail = false }: {
    fill: string; stroke: string; text: string; size: number; mono: boolean; axis: 'x' | 'y'; detail?: boolean;
  },
): Chip {
  const t = String(text);
  return {
...
function drawChip(c: Chip): string {
  return (
...
function contentBounds(
  graph: Graph,
  placed: Placed,
  groups: { hulls: GroupHull[]; usable: boolean },
  chips: Chip[],
...
function deviceCard(
  ...
  graph: Graph,
  theme: Theme,
  { zoneBadge, interactive, show = detailFlags('full') }: { zoneBadge: boolean; interactive: boolean; show?: DetailFlags },
): string {
 ...
 */
export function usedPorts(node: GraphNode, graph: Graph): { port: string | null; media: string }[] {
  return graph.edges
...
function portStrip(n: GraphNode, graph: Graph, box: PlacedBox, theme: Theme): string {
 ...
 */
function externalCard(n: GraphNode, box: PlacedBox, theme: Theme, interactive: boolean, show: DetailFlags): string {
...
function dataCard(n: GraphNode, box: PlacedBox, theme: Theme, interactive: boolean): string {
...
export function legendRow(graph: Graph, theme: Theme, x: number, y: number): string {
  let cx = x;
...
export function titleBlockEl(
  graph: Graph,
  theme: Theme,
  x: number,
  y: number,
  w: number,
  placed: { source: string },
): string {
  const meta = graph.meta ?? ({} as Graph['meta']);
...
function midpointOf(pts: Point[]): Point & { dx: number; dy: number } {
  let total = 0;
