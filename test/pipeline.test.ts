...
import { layoutGraph, groupHulls } from '../src/lib/core/layout/index.ts';
import { freezeLayout } from '../src/lib/core/layout/freeze.ts';
import { renderSvg, usedPorts } from '../src/lib/core/render2d/svg.ts';
...
const EXAMPLE = new URL('../examples/iac-lab.netlas.yaml', import.meta.url).pathname;
...
test('freeze writes coordinates back and the model still validates', async () => {
  const dir = mkdtempSync(join(tmpdir(), 'netlas-'));
  const file = join(dir, 'm.netlas.yaml');
  ...
  assert.ok(readFileSync(file, 'utf8').includes('# netlas example'));
...
test('a switch faceplate shows the same ports as the flat port strip', async () => {
  ...
  assert.equal(ports.length, 3, 'sw-core-1 has three cabled ports in the example (FW uplink + two access switches)');
