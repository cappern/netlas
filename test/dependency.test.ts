...
import { parseModel, resolveDepEndpoint, indexModel } from '../src/lib/core/model/load.ts';
import { loadModel } from '../src/lib/core/model/load-file.ts';
import { validateModel } from '../src/lib/core/model/validate.ts';
import { deriveLayer, layersFor, LAYERS, ALL_LAYERS } from '../src/lib/core/model/derive.ts';
import { buildImpactMap } from '../src/lib/core/model/impact.ts';
import { layoutGraph } from '../src/lib/core/layout/index.ts';
import { renderSvg } from '../src/lib/core/render2d/svg.ts';
import { getTheme, THEME_IDS } from '../src/lib/core/theme/themes.ts';

const model = loadModel(new URL('../examples/ise-dependencies.netlas.yaml', import.meta.url).pathname);
...
test('a layer appears only when the model carries the facts to draw it', () => {
  ...
  // The devices are still physical facts, so L1 draws them (unconnected)
  // alongside the dependency view; the other two stay absent.
...
`);
  assert.deepEqual(layersFor(portfolio), ['l1', 'dep']);

  // With no devices at all there is nothing to place, so L1 stays absent too.
  const empty = parseModel(`
meta: { title: T }
devices: []
dependencies: []
`);
  assert.deepEqual(layersFor(empty), []);
...
test('a model with no dependencies produces an empty impact map', () => {
  const lab = loadModel(new URL('../examples/iac-lab.netlas.yaml', import.meta.url).pathname);
