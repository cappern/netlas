...
const model = loadModel(new URL('../examples/enterprise-dc.netlas.yaml', import.meta.url).pathname);
...
test('the pointer is not captured before a gesture is known to be a drag', () => {
  ...
  const src = readFileSync(new URL('../src/lib/core/viewer/viewer.js', import.meta.url), 'utf8');
